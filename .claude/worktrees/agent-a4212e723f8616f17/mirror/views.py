...
def _update_brain_async(session_pk, user_pk):
    ...
    try:
        ...
        if nuevo_contenido and len(nuevo_contenido) > 50:
            ...
            import re as _re_p
            m_check = _re_p.search(r'## Patrones que he notado(.*?)(?=\n## |\Z)', nuevo_contenido, _re_p.DOTALL)
            if m_check and m_check.group(1).strip() and '(Aún sin' not in m_check.group(1):
...
def espejo_home(request):
    ...
    hace_40 = tz.now() - timedelta(days=40)
    pregunta_retorno = (
        ConflictSession.objects.filter(
            user=request.user,
            status='archived',
            pregunta_cierre__gt='',
...
def espejo_archivar(request, pk):
    ...
    if not sesion.pregunta_cierre:
        ...
        if q:
            ...
            sesion.save(update_fields=['pregunta_cierre'])
    threading.Thread(
