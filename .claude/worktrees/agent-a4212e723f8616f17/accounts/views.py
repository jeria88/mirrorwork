...
def mapa_interior(request):
    ...
    if mem and mem.contenido:
        ...
        if m:
            ...
            patrones = [p.lstrip('-• ').strip() for p in bloque.split('\n') if p.strip() and p.strip() not in ['(Aún sin sesiones registradas.)', '(Sin observaciones aún.)']]

    # Feature 5 — Resonancias colectivas
    from datetime import timedelta as _td
    from django.utils import timezone as _tz
    from mirror.models import ConflictSession as _CS
    import re as _re2

    semana_atras = _tz.now() - _td(days=7)
    descripciones = list(
        _CS.objects.filter(created_at__gte=semana_atras)
        .exclude(conflict_description='')
        .values_list('conflict_description', flat=True)[:300]
    )
    _stopwords_res = {'de','la','el','en','un','una','que','es','y','a','los','las','por',
                      'para','con','como','su','se','al','del','lo','mi','me','te','no',
                      'si','más','pero','hay','tiene','tengo','siento','cuando','porque',
                      'todo','nada','algo','muy','bien','mal','quiero','hacer','ser'}
    freq = {}
    for desc in descripciones:
        for w in _re2.findall(r'\b[a-záéíóúüñ]{5,}\b', desc.lower()):
            if w not in _stopwords_res:
                freq[w] = freq.get(w, 0) + 1
    resonancias = [{'palabra': w, 'count': c} for w, c in sorted(freq.items(), key=lambda x: -x[1])[:5] if c >= 2]
    total_activos = _CS.objects.filter(created_at__gte=semana_atras).values('user').distinct().count()
...
def diario(request):
    ...
    for key, group in groupby(events, key=_month_key):
        grp_list = list(group)
        if grp_list and grp_list[0]['date']:
            label = grp_list[0]['date'].strftime('%B %Y').capitalize()
        else:
            label = 'Sin fecha'
